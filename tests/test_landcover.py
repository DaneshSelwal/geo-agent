...
from app.analysis.landcover import (
    ...
    build_landcover_histogram,
    format_landcover_distribution,
    build_valid_coverage,
    DW_DATASET,
)
...
def test_build_landcover_collection(mock_image_collection):
    ...
    mock_filtered_date.size.return_value = MagicMock()
    ...
    assert count.getInfo() == 42

@patch('ee.Reducer.frequencyHistogram')
def test_build_landcover_histogram_populated(mock_histogram):
    ...
    # 1=trees, 4=crops
    mock_image.select.return_value.reduceRegion.return_value.get.return_value = {
        '1': 300,
        '4': 100
    }

    histogram = build_landcover_histogram(mock_image, mock_aoi, 10)
...
@patch('ee.Reducer.frequencyHistogram')
def test_build_landcover_histogram_empty(mock_histogram):
    mock_image = MagicMock(spec=ee.Image)
    mock_aoi = MagicMock(spec=ee.Geometry)
...
def test_build_valid_coverage(mock_mean):
    mock_image = MagicMock(spec=ee.Image)
    mock_aoi = MagicMock(spec=ee.Geometry)
    ...
    coverage = build_valid_coverage(mock_image, mock_aoi, 10)
...
@patch('ee.Reducer.mean')
def test_build_valid_coverage_none(mock_mean):
    mock_image = MagicMock(spec=ee.Image)
    mock_aoi = MagicMock(spec=ee.Geometry)
...
@patch('app.analysis.landcover.ee.Dictionary')
@patch('app.analysis.landcover.build_valid_coverage')
@patch('app.analysis.landcover.build_landcover_histogram')
@patch('app.analysis.landcover.build_landcover_collection')
def test_analyze_landcover_no_images(mock_build, mock_hist, mock_cov, mock_dict):
    mock_build.return_value = (MagicMock(), MagicMock())
    mock_dict.return_value = MagicMock()
    mock_dict.return_value.getInfo.return_value = {'source_image_count': 0}
    mock_aoi = MagicMock(spec=ee.Geometry)
...
@patch('app.analysis.landcover.build_landcover_histogram')
@patch('app.analysis.landcover.build_landcover_collection')
def test_analyze_landcover_happy_path(mock_build, mock_build_hist, mock_build_cov, mock_dict):
    mock_collection = MagicMock()
    mock_composite = MagicMock()
    mock_collection.select.return_value.mode.return_value = mock_composite

    mock_build.return_value = (mock_collection, MagicMock())
    mock_collection.size.return_value = MagicMock()
    mock_dict.return_value = MagicMock()
    mock_dict.return_value.getInfo.return_value = {'source_image_count': 42, 'histogram': {'1': 80, '2': 20}, 'coverage': 0.95}
    ...
    mock_build_hist.assert_called_once_with(composite=mock_composite, aoi=mock_aoi, scale=scale)
    mock_build_cov.assert_called_once_with(image=mock_composite, aoi=mock_aoi, scale=scale)
